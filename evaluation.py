""" compute the precision and recall for with the test result (a text file generated by demo_dir_for_pr.py).
author: meringue
date: 2018/6/9
"""
import numpy as np 
...
def process_predicts(predict_txt):
    ...
    for predict_row in predict:
        ...
        for obj_index in range(obj_num):
            ...
            if not img_dict.has_key(obj_name):
                img_dict[obj_name] = []
...
def compute_precision_recall(label_txt, predict_txt, iou1=0.5):
    ...
    for img_name, obj_label in label_dict.items():
        ...
        for obj, coords in obj_label.items():
            count = 0
            if obj_predict.has_key(obj):
                img_num_per_cls[obj] +=1
                for coord1 in coords:
                    for coord2 in obj_predict[obj]:
                        if compute_iou(coord1, coord2)>=iou1:
                            count +=1
                pre[obj] += count/len(obj_predict[obj])
                rec[obj] += count/len(coords)

    for cls_name in cls_names:
        pre_rec[cls_name] = [pre[cls_name]/img_num_per_cls[cls_name], rec[cls_name]/img_num_per_cls[cls_name]]
