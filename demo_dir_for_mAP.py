""" a demo for testing the images in a directory (for computing mAP).
author: meringue
date: 2018/6/9
"""
import sys
...
def write_predicts_to_txt(img_name, predicts_dict, out_dir):
	"""write the predict results to a txt file.
	Args:
		img_name: image name.
		predicts_dict: {"stick": [[x1, y1, x2, y2, scores1], [...]]}.
		out_dir: write directory.
	"""
	txt_name = os.path.split(img_name)[-1].split(".")[0] + ".txt"
	...
	with open(txt_path, "a") as f:
		for obj_name, bbox in predicts_dict.items():
			...
			for coord in bbox:
				...
				score = np.round(coord[-1], 3)
				row_data = obj_name + " " + str(score) + " " + str(x1) + " " + str(y1) + " " + str(x2) + " " + str(y2) + "\n"
				f.write(row_data)
...
if __name__ == '__main__':
	...
	root_dir = "/home/meringue/Documents/stick_cup_pen_detection_tensorflow/tensorflow-yolo-python2.7"
	os.chdir(root_dir)
    
	test_img_folder = os.path.join(root_dir, "./data/test_image/JPEGImages")
	out_txt_dir = "./data/test_result_txt"
	...
	saver.restore(sess, 'models/train20180410/model.ckpt-100000')
	...
	for img_name in img_names:
		...
		write_predicts_to_txt(img_name, current_predicts_dict, out_txt_dir)
