...
class YoloTinyNet(Net):
  ...
  def inference(self, images):
    ...
    local3 = self.local('local3', local2, 4096, self.cell_size * self.cell_size * (self.num_classes + self.boxes_per_cell * 5), leaky=False, train=True)
  ...
  def body1(self, num, object_num, loss, predict, labels, nilboy):
    ...
    min_x = tf.maximum(0, tf.cast(tf.floor(min_x), tf.int32))
    min_y = tf.maximum(0, tf.cast(tf.floor(min_y), tf.int32))

    max_x = tf.minimum(tf.cast(self.image_size, tf.int32), tf.cast(tf.ceil(max_x), tf.int32))
    max_y = tf.minimum(tf.cast(self.image_size, tf.int32), tf.cast(tf.ceil(max_y), tf.int32))
