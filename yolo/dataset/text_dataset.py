...
class TextDataSet(DataSet):
  ...
  def batch(self):
    ...
    for i in range(self.batch_size):
      ...
      #image, label = self._augmentation(image, label, object_num)
      images.append(image)
      labels.append(label)
  ...
  def _augmentation(self, image, labels, object_num):
    """image augumentation
    Args:
      image: 3-D ndarray
      labels: 2-D list [self.max_objects, 5] (xcenter, ycenter, w, h, class_num)
    Return:
      ...
      labels: 2-D list [self.max_objects, 5] (xcenter, ycenter, w, h, class_num)
    """
    classes_name_list = ["stick"]
    h, w, _ = image.shape
    max_objects = len(labels)
    classes_num = []
    classes_name = []
    coords = []
    new_labels = []
    for i in range(max_objects):
      new_labels.append([0, 0, 0, 0, 0])

    for i in range(object_num):
      label = labels[i]
      class_num = label[-1]
      classes_num.append(int(class_num))
      classes_name.append(classes_name_list[int(class_num)])
      coords.append(label[:-1])

      ## brightness
    image = tl.prepro.brightness(image, gamma=0.4, gain=1, is_random=True)
      ## flip
    image, coords = tl.prepro.obj_box_left_right_flip(image, coords, 
        is_rescale=False, is_center=True, is_random=True)
      ## resize  
    image, coords = tl.prepro.obj_box_imresize(image, coords, 
        size=[int(1.3*h), int(1.3*w)], is_rescale=False)
      ## crop
    image, classes_num, coords = tl.prepro.obj_box_crop(image, classes_num, coords, 
        wrg=w, hrg=h, is_rescale=False, is_center=True, is_random=True)
      ## shift
    image, classes_num, coords = tl.prepro.obj_box_shift(image, classes_num, coords, 
        wrg=0.1, hrg=0.1, is_rescale=False, is_center=True, is_random=True)
      ## zoom
    image, classes_num, coords = tl.prepro.obj_box_zoom(image, classes_num, coords, 
      zoom_range=(1.3, 0.7), is_center=True, is_random=True)
    ## show result
    classes_list = []
    for i in range(len(classes_num)):
      class_name = classes_name_list[classes_num[i]]
      classes_list.append(class_name)

    for i in range(len(classes_num)):
        xcenter, ycenter, width, height = coords[i]
        class_num = classes_num[i]
        new_labels[i] = [int(xcenter), int(ycenter), int(width), int(height), class_num]
